...
The diff corpus is the reusable half of a fleet adoption.
It preserves not only the final API call, but the exact, behavior-tested transformation that replaced an older structure.
...
Dependency-floor changes, generated lockfile updates, and formatter-only changes belong in separate commits unless they are indivisible from the transformation.
...
It refuses dirty tracked worktrees, identical revisions, empty patches, symlink destinations, and patches larger than 2 MB.
For a larger coherent change, split the implementation into atomic commits rather than truncating the evidence.
...
> Replace **old conceptual owner** with **new canonical owner**, preserving **observable contract** and removing **obsolete parallel structure**.
