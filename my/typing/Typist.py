...
class Typist(pyd.BaseModel):
    ...
    MATCH_CACHE: ClassVar[NestedCache[tuple[str, str], bool]] = NestedCache(
        signature=(str, str)
    )
    ...
    def sort_options(cls, data: object, *options: MyType) -> list[MyType]:
        """Arbitrarily determine how "fitting" a destination type is for coercion the given source.

        Args:
            data: The source type.
            target: The target type.
        Returns:
            An integer score, where higher is better (i.e. "more fitting").
        """
        return list(
            sorted(options, key=lambda opt: cls._score_option(data, opt), reverse=True)
        )
    ...
    def _derive_container(
        cls, old: MyType, new_origin: type[types.GenericAlias]
    ) -> MyType:
        if old.origin and old.args:
            ...
            new_src = new_origin[
                typing.Unpack[*tuple(arg.src_type for arg in old.args)]
            ]
        else:
            new_src = new_origin
    ...
    def _to_literal(self, data: object, target: MyType) -> object | None:
        ...
        if target.origin is Literal:
            ...
            if data is not None and len(data) == len(target.args):
                ret = target.origin(
                    it.starmap(self.cast, zip(data, target.args, strict=True))
                )
    ...
    def _time_to_atomic(
        self, data: Time, target: type[AtomicType]
    ) -> AtomicType | None:
    ...
    def _str_to_atomic(self, data: str, target: type[AtomicType]) -> AtomicType | None:
        data = data.strip()
        if issubclass(target, int) and self.RGXS["int"].fullmatch(data):
            return target(data)
        elif issubclass(target, float) and self.RGXS["float"].fullmatch(data):
            return target(data)
    ...
    def _cast_members(items: Iterable[tuple[str, Any]], target: type) -> dict[str, Any]:
        ...
        return {
            key: typist.flexcast(val, annotations.get(key, None)) for key, val in items
        }
    ...
    def _num_to_time(
        self, data: int | float, target: type[TimeType]
    ) -> TimeType | None:
        if issubclass(target, datetime):
    ...
    @staticmethod
    def type_partition(
        container: Iterable, t0: type[C], t1: type[T]
    ) -> tuple[list[C], list[T]]:
    ...
    def match_instances(
        self, inst0: object, inst1: object, intersect: bool = False
    ) -> bool:
        return self.match(type(inst0), type(inst1), intersect)
    ...
    def seek_usage(self, lhs: TypeArg, rhs: type | MyType) -> bool:
        ...
        if inspect.isclass(rhs):
            # III.ii.a.
            return any(
                self.seek_usage(t0, ann) for ann in ut.instance_fields(rhs).values()
            )
        elif t1.val_type:
    ...
    def cast_file_data(self, data: RawJsonData, tvar: type[F]) -> F:
        ...
        # III. Catching mistmatched collections
            ...
            if len(data) == 1 and isinstance(
                (first := mi.first(data.values())), Series
            ):
                return tvar(first)
            else:
                return tvar([data])
    ...
    def serialize(
        self,
        data: object,
        cases: dict[type | Callable[[object], bool], Callable[[object], Any]]
        | None = None,
        full: bool = False,
    ) -> Any:
        ...
        if isinstance(data, pyd.BaseModel):
            if cases:
                ...
                for case, handler in cases.items():
                    if (isinstance(case, type) and isinstance(data, case)) or case(
                        data
                    ):
                        return handler(data)
    ...
    def distill(self, models: list[dict], exclude: set[str] | None = None) -> dict:
        ...
        """
        assert len(models) > 1, (
            f"At least two models are required to distill, got {len(models)}."
        )
        ...
        for key in set(base.keys()):
            ...
            if set(map(type, filter(bool, restvals))) != {_type}:
                ...
                if shared := set(
                    filter(lambda item: ut.all_has_all(restvals, item), base[key])
                ):
                    for model in models:
                        if model[key]:
                            model[key] = _type(
                                it.filterfalse(shared.__contains__, model[key])
                            )
                        if not model[key]:
                            del model[key]
                    distillate[key] = _type(
                        sorted(set(distillate.get(key, [])) | shared)
                    )
    ...
    def to_yaml(self, data: FileData, wrap: bool = False, **kwargs: Any) -> str:
    ...
    def to_json(self, data: FileData, wrap: bool = False, **kwargs: Any) -> str:
        obj = self.serialize(data)
    ...
    def to_toml(self, data: FileData, wrap: bool = False, **kwargs: Any) -> str:
        obj = self.serialize(data)
    ...
    def to_pickle(self, data: FileData) -> bytes:
        obj = self.serialize(data)
    ...
    def invocable(
        ...
        if len(args) == 1:
            if any(map(cls._param_is_keyword, params)) and (
                items := ut.map_items(args[0])
            ):
