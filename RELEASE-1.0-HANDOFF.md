...
Local proof is literal below. The two remote rows are explicit pre-merge gates,
not soft passes: they can exist only after the review branch is submitted.
...
| GitLab Basis pipeline          | exact merge-request pipeline                                                                                          | Pending external gate: the pipeline is created only after branch submission; merge is prohibited until it is green.                                                                                                        |
| Basis review surface           | submitted merge request                                                                                               | Pending branch submission; the maintainer-owned dirty primary checkout is intentionally untouched.                                                                                                                         |
