...
class TestRegexStore:
    ...
    def test_parse_mark(
        self, mark: str, expected: tuple[str, str, str], store: RegexStore
    ):
    ...
    def test_compose_tuple(
        self, definition: RegexVal, expected: str, store: RegexStore
    ):
    ...
    def test_parse_tuple__length_4(self, store: RegexStore):
        mark, children, pre, suf = store._parse_tuple(
            ("|:", "PREFIX", ["a", "b"], "SUFFIX")
        )
        assert mark == "|:"
        assert children == ["a", "b"]
    ...
    def test_match(
        self, store: RegexStore, pattern_name: str, text: str, should_match: bool
    ):
        _ = store.load
    ...
    def test_automatch__aliases(
        self, store: RegexStore, func: str, alias: str, text: str
    ):
        _ = store.load
    ...
    def test_tree__importas_example(
        ...
        true_negatives, false_positives = map(
            list, mi.partition(_fn, negative_examples)
        )
