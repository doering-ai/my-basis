...
The `Markdown` class implements a complete hierarchical document model for markdown files.
...
This indexing system enables efficient path tracing and node lookup operations.
...
Prose content is stored in `Buffer` objects (from `my.types`), enabling efficient text manipulation operations.
